...
## Legal Query
In M22 the /legaltags:query API was introduced.
This new POST /legaltags:query API takes a payload of `{"query": ["attribute=value"]}` and is implemented as partial match ignoring case.
...
For expirationDate you can do a string match (even partial) or give a date range:

```
{"query": ["expirationDate between (2023-01-01, 2024-12-31)"]}
```
...
Examples search by extension properties:
...
curl -X 'POST' \
  ...
  -H 'data-partition-id: osdu' \
  -H 'Content-Type: application/json' \
  -d '{"query":["AgreementIdentifier=test"]}'
```
