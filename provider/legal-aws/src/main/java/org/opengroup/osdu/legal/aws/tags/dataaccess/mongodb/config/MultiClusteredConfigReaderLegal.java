...
public class MultiClusteredConfigReaderLegal implements MultiClusteredConfigReader {
    ...
    @Autowired
    public MultiClusteredConfigReaderLegal(SSMManagerUtil ssmUtil) {
    ...
    public MongoProperties readProperties(PartitionInfoAws partitionInfoAws) {
        ...
        String ssmPathPattern = "/osdu/%s/tenants/%s/legal/mongoDB";
        String secretManagerPathPattern = "/osdu/%s/tenants/%s/legal/mongoDB/credentials-arn";
        ...
        if (tableParams.size() <= 0) {
            ...
            return MongoProperties.builder()
                    ...
                    .enableTLS(enableTLS)
                    .databaseName(this.environment + "_osdu_legal")
